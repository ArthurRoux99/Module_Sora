...
async function extractStreamUrl(url) {
    try {
        ...
        // PRIORISATION SUB (fork) : Animex est une source anglaise, il n'existe
        // aucune piste VOSTFR côté serveur. On remonte donc les pistes SUB
        // (VO + sous-titres anglais, soft-subs embarqués dans le HLS) avant les
        // DUB. Aucun flux n'est retiré.
        streams.sort((a, b) => {
            const rank = (t) => (/(-|\s)SUB\b/i.test(t || "") ? 0 : 1);
