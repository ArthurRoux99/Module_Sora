...
// ==========================================
// 🗄️ TRACKER SUPABASE — NEUTRALISÉ (fork VOSTFR)
// ==========================================
// L'original POSTait chaque recherche et chaque lecture (titre, épisode,
// serveurs) vers un Supabase tiers appartenant à l'auteur amont. Mesuré :
// 4 requêtes sortantes par exécution. Aucune fonctionnalité du module n'en
// dépend — la fonction est conservée en no-op pour ne pas toucher aux ~15
// sites d'appel existants.
async function sendSupabaseLog(moduleName, actionType, dataPayload) {
    return; // no-op : aucune donnée ne quitte l'appareil
}
...
async function extractStreamUrl(url) {
    ...
    try {
        ...
        for (let embed of allEmbeds) {
            ...
            // 7. DETECTEUR UNIVERSEL (packer + m3u8/mp4 en clair)
            //
            // CORRECTIF (fork VOSTFR) : l'original exigeait '/vidhide/' ou un
            // packer pour tenter l'extraction. Conséquence mesurée sur
            // Frieren S1 VOSTFR ep1 : ansembed.net (0 packer, pas de
            // '/vidhide/') était IGNORÉ alors que sa page contient un m3u8
            // 1080p qui répond 200. On tente donc l'extraction sur toute page,
            // et on nomme le serveur d'après son vrai domaine plutôt que de
            // tout étiqueter "Vidhide" (smoothpre.com était annoncé "Vidhide"
            // alors que son hôte acek-cdn.com ne résout même pas en DNS).
            else {
                try {
                    const req = await fetchv2(embedUrl, { "Referer": "https://anime-sama.to/" }, "GET");
                    const html = await req.text();
                    ...
                    if (streamUrl) {
                        ...
                        let originUrl = originMatch ? originMatch[1] + "/" : "https://anime-sama.to/";
                        let hostMatch = embedUrl.match(/https?:\/\/(?:www\.)?([^\/:]+)/i);
                        let serverName = hostMatch ? hostMatch[1].split('.')[0] : "Embed";
